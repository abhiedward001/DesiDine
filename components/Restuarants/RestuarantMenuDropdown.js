...
function RestuarantMenuDropdown({ data ,toggle, setToggle,setShowFlashMessage}) {
    ...
    return (
        ...
        <div className="w-6/12 mx-auto bg-gray-200 shadow-md rounded-sm my-4 p-4 ">
            ...
            <div>
                ...
                <ul>
                    {data.itemCards.map((item) => {
                        return (<li key={item?.card?.info?.id} >{toggle && <FoodInfo  itemValue={item?.card?.info}  setShowFlashMessage={setShowFlashMessage}></FoodInfo>}</li>)
                    })}
                </ul>
            </div>
            {/* {toggle && toggleData} */}
